...
echo "denis: $ROBOMAKER_CONTAINERS"
...
echo "DR_VIEWER_HTML:  $DR_VIEWER_HTML"
...
then
  ...
  echo "1 - docker stack deploy -c $COMPOSE_FILES $STACK_NAME"

else
  docker-compose -f $COMPOSE_FILES -p $STACK_NAME --log-level ERROR up -d 
  echo "2 - docker-compose -f $COMPOSE_FILES -p $STACK_NAME --log-level ERROR up -d "
fi
